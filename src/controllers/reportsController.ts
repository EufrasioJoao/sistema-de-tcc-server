...
export async function getSystemStatistics(
    ...
    try {
        ...
        if (user.role !== UserRoles.ADMIN && user.role !== UserRoles.SISTEM_MANAGER) {
            res.status(403).json({
                success: false,
                message: "Acesso negado. Apenas administradores podem visualizar estatísticas do sistema",
            });
            return;
        }
...
export async function getTccReports(
    ...
    try {
        ...
        const supervisorIds = topSupervisors.map(item => item.supervisorId);
        ...
        const topSupervisorsFormatted = topSupervisors.map(item => {
            const supervisor = supervisors.find(s => s.id === item.supervisorId);
            return {
                supervisorId: item.supervisorId,
                supervisorName: supervisor ? `${supervisor.first_name} ${supervisor.last_name}` : 'Supervisor não encontrado',
                count: item._count.id,
            };
        });
        ...
        res.json({
            ...
            data: {
                ...
                topSupervisors: topSupervisorsFormatted,
                recent: recentTccs,
            },
        });
    } catch (error) {
...
export async function getUserActivityReports(
    ...
    try {
      ...
      `,
        ...
        const fileIds = mostAccessedFiles.map(item => item.file_id).filter(Boolean) as string[];
        ...
        const mostActiveUsersFormatted = mostActiveUsers.map(item => {
            ...
            return {
                ...
                userName: userData ? `${userData.first_name} ${userData.last_name}` : 'Usuário não encontrado',
        ...
        const mostAccessedFilesFormatted = mostAccessedFiles.map(item => {
            ...
            return {
                fileId: item.file_id,
                fileName: fileData?.displayName || fileData?.filename || 'Arquivo não encontrado',
...
export async function getCourseReports(
    ...
    try {
        ...
        const [
            ...
            db.$queryRaw<Array<{ id: string, name: string, supervisor_count: number }>>`
                SELECT 
                    c.id,
                    c.name,
                    CAST(COUNT(DISTINCT t.supervisorId) as SIGNED) as supervisor_count
                FROM courses c
                LEFT JOIN tccs t ON c.id = t.courseId AND t.deletedAt IS NULL
...
export async function getAuthorReports(
    ...
    try {
        ...
        const [
            ...
            db.$queryRaw<Array<{ id: string, firstName: string, lastName: string, email: string, supervisor_count: number }>>`
                SELECT 
                    s.id,
                    s.firstName,
                    s.lastName,
                    ...
                    CAST(COUNT(DISTINCT t.supervisorId) as SIGNED) as supervisor_count
                FROM students s
                LEFT JOIN tccs t ON s.id = t.authorId AND t.deletedAt IS NULL
...
export async function getKeywordReports(
    ...
    try {
        ...
        const tccsWithKeywords = allTccs.filter(tcc =>
            tcc.keywords && tcc.keywords.trim() !== ''
        );
        ...
        tccsWithKeywords.forEach(tcc => {
            if (tcc.keywords) {
                const keywords = tcc.keywords.split(',').map(k => k.trim().toLowerCase());
        ...
        const keywordsByCourseFormatted = Array.from(keywordsByCourse.entries()).map(([courseName, keywords]) => ({
            courseName,
...
/**
 * Get theme repetition reports
 */
export async function getThemeReports(
    req: AuthRequest,
    res: Response
): Promise<void> {
    try {
        ...
        // Get all TCCs with titles and keywords
        ...
        // Theme detection based on actual word frequency in titles
        const wordFrequency = new Map<string, number>();
        const stopWords = ['de', 'da', 'do', 'para', 'com', 'em', 'na', 'no', 'e', 'o', 'a', 'um', 'uma', 'dos', 'das', 'por', 'ao', 'à'];

        // Extract meaningful words from titles
        tccs.forEach(tcc => {
            const words = tcc.title.toLowerCase()
                .split(/\s+/)
        ...
        // Only consider words that appear more than once as themes
        ...
        // Detect themes based on common words in titles
        tccs.forEach(tcc => {
            const titleWords = tcc.title.toLowerCase().split(/\s+/);
            const foundThemes = commonWords.filter(theme =>
                titleWords.some(word => word.includes(theme) || theme.includes(word))
            );
            ...
            foundThemes.forEach(theme => {
                ...
                themeData.tccs.push({
                    id: tcc.id,
                    title: tcc.title,
                    authorName: `${tcc.author?.firstName} ${tcc.author?.lastName}`,
                    courseName: tcc.course?.name || 'Curso não encontrado',
                    year: tcc.year,
                });
                themeData.courses.add(tcc.course?.name || 'Curso não encontrado');
                themeData.years.add(tcc.year);
            });
        });

        // Convert to arrays and sort
        const mostRepeatedThemes = Array.from(themeMap.entries())
            .map(([theme, data]) => ({
                theme,
                count: data.count,
                courseCount: data.courses.size,
                yearSpread: data.years.size,
                tccs: data.tccs.slice(0, 5), // Limit to 5 TCCs per theme
            }))
            .filter(item => item.count > 1) // Only themes that appear more than once
            .sort((a, b) => b.count - a.count)
            .slice(0, 15);

        // Themes by course
        const themesByCourse = new Map<string, Map<string, number>>();
        tccs.forEach(tcc => {
            const courseName = tcc.course?.name || 'Curso não encontrado';
            if (!themesByCourse.has(courseName)) {
                themesByCourse.set(courseName, new Map());
            }
            const courseThemes = themesByCourse.get(courseName)!;
            ...
            foundThemes.forEach(theme => {
                courseThemes.set(theme, (courseThemes.get(theme) || 0) + 1);
            });
        });

        const themesByCourseFormatted = Array.from(themesByCourse.entries()).map(([courseName, themes]) => ({
            courseName,
            topThemes: Array.from(themes.entries())
                ...
                .sort((a, b) => b.count - a.count)
                .slice(0, 8),
        })).filter(course => course.topThemes.length > 0);

        // Themes by year
        const themesByYear = new Map<number, Map<string, number>>();
        tccs.forEach(tcc => {
            if (!themesByYear.has(tcc.year)) {
                themesByYear.set(tcc.year, new Map());
            }
            const yearThemes = themesByYear.get(tcc.year)!;
            ...
            foundThemes.forEach(theme => {
                yearThemes.set(theme, (yearThemes.get(theme) || 0) + 1);
            });
        });

        const themesByYearFormatted = Array.from(themesByYear.entries())
            .map(([year, themes]) => ({
                year,
                topThemes: Array.from(themes.entries())
                    .map(([theme, count]) => ({ theme, count }))
            ...
            .filter(year => year.topThemes.length > 0)
            .sort((a, b) => b.year - a.year);

        res.json({
            success: true,
            data: {
                mostRepeated: mostRepeatedThemes,
                byCourse: themesByCourseFormatted,
                byYear: themesByYearFormatted,
    ...
    try {
        ...
        const storageByUploaderFormatted = storageByUploader.map(item => {
            ...
            return {
                ...
                uploaderName: uploaderData ? `${uploaderData.first_name} ${uploaderData.last_name}` : 'Usuário não encontrado',
